...
class GuestMolecule(IMolecule):
    ...
    def calculate_binding_sites(self):
        ...
        if len(guest_atom_distances) == 1:
            ...
            distance_1_2 = None
            linear_guest = True
            reversible_guest = True if len(PGA(self).get_equivalent_atoms()["eq_sets"][1]) == 1 else False
        # If there are more than two atoms, take the closest three
            ...
            distance_1_2 = np.linalg.norm(self.cart_coords[guest_atom_distances[1][1]] -
                                          self.cart_coords[guest_atom_distances[2][1]])
            linear_guest = True if str(PGA(self).get_pointgroup()) == ("D*h" or "C*v") else False
            # If the non-central atom has another equivalent atom
            reversible_guest = True if len(PGA(self).get_equivalent_atoms()["eq_sets"][1]) == 2 else False
        ...
        for binding_site in binding_sites:
            for n, atom in enumerate(binding_site):
                ...
                if n == 1:
                    dist_vector = atom[1]
                    align_coords = (self.get_structure()).lattice.get_fractional_coords((self.get_structure()).lattice.get_cartesian_coords(central_coords) + dist_vector)
                    ...
                    if len(guest_atom_distances) == 3 and linear_guest and reversible_guest:
                        orient_coords = (self.get_structure()).lattice.get_fractional_coords((self.get_structure()).lattice.get_cartesian_coords(central_coords) - dist_vector)
        ...
        if atom_at_center_of_mass is True:
            ...
            if site_at_com is not None:
                if os.path.exists(probability_file_com):
                    try:
                        ...
                        raise Exception(f'Error loading cube file: {probability_file_com}')
                else:
                    raise FileNotFoundError('''Error Code: UNAVAILABLE_CUBE_FILE
            Error Description: Unavailable Probability File
            Error Message: We apologize, but we couldn't locate any available unfolded or folded probability file required for this operation.\n''')
            ...
            Error Message: We apologize, but we couldn't locate any available unfolded or folded probability file required for this operation.\n''')
                ...
                if os.path.exists(probability_file_guest):
                    try:
                        self.cube = VolumetricData.from_cube(probability_file_guest)
                        localdata = self.cube.data['total']
                        localdata = localdata / np.sum(localdata)
                        self._datapoints = localdata
                    except Exception as e:
